    ...
    $saveBtn.on('click', function() {
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                collectionHtml += '<p class="card-text">Items: ' + (collection.items ? collection.items.length : 0) + '</p>';
                collectionHtml += '<div class="d-flex justify-content-between">';
                collectionHtml += '<a href="/collections/' + collection.id + '" class="btn btn-primary">View Collection</a>';
                collectionHtml += '<button class="btn btn-danger delete-collection-btn" data-id="' + 
                    collection.id + '" data-name="' + collection.name + '">Delete</button>';
                collectionHtml += '</div>';
                ...
                var $newDeleteBtn = $('#collectionsContainer .delete-collection-btn').last();
                $newDeleteBtn.on('click', function() {
                    ...
                    $('#deleteCollectionModal .modal-body').html(
                        ...
                        '<meta id="collection-id-to-delete" name="collection-id-to-delete" content="' + collectionId + '">'
                    );
...
function initCollectionDetailPage() {
    ...
    $('#mediaSearchInput').on('input', function() {
        ...
        $('.media-item-card').each(function() {
            ...
            if (itemName.indexOf(searchTerm) > -1 || itemDesc.indexOf(searchTerm) > -1) {
                $(this).show();
            } else {
                $(this).hide();
            }
    ...
    function loadMediaItems() {
        $mediaSelectorContainer.html('<div class="col-12 text-center"><div class="spinner-border" role="status"><span class="visually-hidden">Loading...</span></div></div>');
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                if (!response.media || response.media.length === 0) {
                    $mediaSelectorContainer.html('<div class="col-12"><div class="alert alert-info">No images found. Please upload images in the Media Library first.</div></div>');
                    return;
                }
                ...
                for (var i = 0; i < response.media.length; i++) {
                    ...
                    mediaHtml += '<div class="card h-100 media-item-card" data-id="' + item.id + '" data-path="' + item.path + '" data-name="' + item.originalname + '" data-description="' + (item.description || '') + '">';
                    mediaHtml += '<img src="' + item.path + '" class="card-img-top" alt="' + item.originalname + '" style="height: 120px; object-fit: cover;">';
                    mediaHtml += '<div class="card-body">';
                    mediaHtml += '<h6 class="card-title text-truncate">' + item.originalname + '</h6>';
                    mediaHtml += '<button type="button" class="btn btn-sm btn-primary select-media-btn" data-path="' + item.path + '">Select</button>';
            ...
            error: function(xhr) {
                $mediaSelectorContainer.html('<div class="col-12"><div class="alert alert-danger">Error loading media: ' + xhr.responseText + '</div></div>');
            }
        });
    }
    ...
    function selectMedia(mediaPath) {
        ...
        previewContainer.html('<img src="' + mediaPath + '" class="img-thumbnail" style="max-height: 100px;">');
    ...
    $('#saveItemBtn').on('click', function() {
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                var $alertContainer = $('<div class="alert alert-success alert-dismissible fade show" role="alert"></div>');
                $alertContainer.text(mode === 'edit' ? 'Item updated successfully!' : 'Item added successfully!');
                $alertContainer.append('<button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>');
                ...
                $('#saveItemBtn').text('Add Item').data('mode', 'add').removeData('item-id');
                ...
                if (mode === 'edit') {
                    ...
                    for (var field in item) {
                        if (field !== 'id' && field !== 'createdAt' && field !== 'updatedAt') {
                            ...
                            if (value.startsWith('/uploads/') || value.startsWith('/public/uploads/')) {
                                $row.append('<td><img src="' + value + '" alt="Media" class="img-thumbnail" style="max-width: 50px; max-height: 50px;"></td>');
                            } else {
                    ...
                    // Add action buttons
                    var actionsHtml = '<td>';
                    actionsHtml += '<button class="btn btn-sm btn-primary edit-item-btn">Edit</button> ';
                    actionsHtml += '<button class="btn btn-sm btn-danger delete-item-btn">Delete</button>';
                    actionsHtml += '</td>';
                    ...
                    if (response.item) {
                        ...
                        if ($('.alert-info').length > 0 && $('.alert-info').text().includes('No items')) {
                            $('.alert-info').remove();
                        ...
                        for (var field in item) {
                            if (field !== 'id' && field !== 'createdAt' && field !== 'updatedAt') {
                                ...
                                // Check if this is a media field
                                if (value.startsWith('/uploads/') || value.startsWith('/public/uploads/')) {
                                    $newRow.append('<td><img src="' + value + '" alt="Media" class="img-thumbnail" style="max-width: 50px; max-height: 50px;"></td>');
                                } else {
                        ...
                        $newRow.find('.edit-item-btn').on('click', function() {
                            ...
                            $('table thead th').each(function(index) {
                                if (index < $('table thead th').length - 1) { // Skip the Actions column
                            ...
                            $row.find('td').each(function(index) {
                                if (index < $row.find('td').length - 1) { // Skip the Actions column
                            ...
                            for (var field in itemData) {
                                ...
                                if ($field.length > 0) {
                                    ...
                                    if ($field.hasClass('media-field-input') || $('#' + field + '_display').length > 0) {
                                        $('#' + field + '_display').val(itemData[field]);
                                        $('#' + field + '_preview').html('<img src="' + itemData[field] + '" class="img-thumbnail" style="max-height: 100px;">');
                                    }
                                }
                            }
                            ...
                            $('#saveItemBtn').text('Update Item').data('mode', 'edit').data('item-id', itemId);
            ...
            error: function(xhr) {
                alert('Error ' + (mode === 'edit' ? 'updating' : 'adding') + ' item: ' + xhr.responseText);
            }
        });
    });
    ...
    $saveBtn.on('click', function() {
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                mediaHtml += '<img src="' + media.path + '" class="card-img-top" alt="' + media.originalname + '" style="height: 150px; object-fit: cover;">';
                mediaHtml += '<div class="card-body">';
                // Add meta tag with media ID
                mediaHtml += '<!-- @mediaId:' + media.id + ' -->';
                mediaHtml += '<h6 class="card-title text-truncate">' + media.originalname + '</h6>';
                mediaHtml += '<p class="card-text small text-muted">' + (media.description || 'No description') + '</p>';
                mediaHtml += '<div class="d-flex justify-content-between">';
                mediaHtml += '<button class="btn btn-sm btn-primary preview-image-btn" data-id="' + media.id + '" data-path="' + media.path + '" data-name="' + media.originalname + '" data-description="' + (media.description || '') + '">Preview</button>';
                mediaHtml += '<button class="btn btn-sm btn-danger delete-image-btn" data-id="' + media.id + '">Delete</button>';
                mediaHtml += '</div>';
                mediaHtml += '</div></div></div>';
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                if ($('#mediaContainer .col-md-3').length === 0) {
                    $('#mediaContainer').html('<div class="col-12"><div class="alert alert-info">No images found. Upload your first image to get started.</div></div>');
                }
            },
            error: function(xhr) {
